...
const URL = "http://localhost:1337/psu-specs/";
...
function PsuSpecs({ id }) {
  const classes = useStyles();
  const [allmant, setAllmant] = useState([]);
  const [natdels, setNatdel] = useState([]);

  useEffect(() => {
    async function fetchData() {
      const request = await axios(`${URL}${id}`);

      setAllmant(request.data.psu_specs_props_allmants);
      setNatdel(request.data.psu_specs_props_natdels);
    }
  ...
  return (
    <TableContainer component={Paper} style={{ marginBottom: "50px" }}>
      <Table className={classes.table} aria-label="customized table">
        ...
        <TableHead className={classes.width}>
          <TableRow>
            <StyledTableCell>Nätdel</StyledTableCell>
            <StyledTableCell></StyledTableCell>
          </TableRow>
        </TableHead>
