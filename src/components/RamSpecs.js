...
const URL = "http://localhost:1337/ram-specs/";
...
function RamSpecs({ id }) {
  ...
  useEffect(() => {
    async function fetchData() {
      ...
      setAllmant(request.data.ram_specs_props_allmants);
      setminne(request.data.ram_specs_props_minnes);
    }
  ...
  return (
    <TableContainer component={Paper} style={{ marginBottom: "50px" }}>
      <Table className={classes.table} aria-label="customized table">
        ...
        <TableHead className={classes.width}>
          <TableRow>
            <StyledTableCell>Minne</StyledTableCell>
            <StyledTableCell></StyledTableCell>
          </TableRow>
        </TableHead>
